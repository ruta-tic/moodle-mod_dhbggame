...
class mod_dhbggame_mod_form extends moodleform_mod {
    function definition() {
        global $CFG, $DB;
        $mform = $this->_form;

        $config = get_config('dhbggame');
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'content', get_string('contentheader', 'dhbggame'));
        $mform->addElement('url', 'externalurl', get_string('externalurl', 'dhbggame'), array('size'=>'60'), array('usefilepicker'=>false));
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'optionssection', get_string('optionsheader', 'dhbggame'));
        ...
        if (count($options) == 1) {
            ...
            $mform->addElement('select', 'display', get_string('displayselect', 'dhbggame'), $options);
            $mform->setDefault('display', $config->display);
            $mform->setAdvanced('display', $config->display_adv);
            $mform->addHelpButton('display', 'displayselect', 'dhbggame');
        }

        if (array_key_exists(RESOURCELIB_DISPLAY_POPUP, $options)) {
            $mform->addElement('text', 'popupwidth', get_string('popupwidth', 'dhbggame'), array('size'=>3));
            if (count($options) > 1) {
            ...
            $mform->addElement('text', 'popupheight', get_string('popupheight', 'dhbggame'), array('size'=>3));
            if (count($options) > 1) {
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_AUTO, $options) or
          ...
          array_key_exists(RESOURCELIB_DISPLAY_FRAME, $options)) {
            $mform->addElement('checkbox', 'printintro', get_string('printintro', 'dhbggame'));
    ...
    function validation($data, $files) {
        ...
        if (empty($data['externalurl'])) {
            ...
            if (empty($url)) {
                ...
                // normal URL
                if (!dhbggame_appears_valid_url($url)) {
                    $errors['externalurl'] = get_string('invalidurl', 'dhbggame');
                }
                ...
                if (!dhbggame_appears_valid_url('http://'.$url)) {
                    $errors['externalurl'] = get_string('invalidurl', 'dhbggame');
                }
            }
        }
